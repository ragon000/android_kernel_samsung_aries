...
enum {
	...
	BATT_TEMP_ADC,			/* Battery Temperature ADC value from RIL */
	BATT_SOC,				/* Battery capacity value from RIL */
	BATT_VOLT,				/* Battery voltage value from RIL */
	BATT_FULL_INTERRUPT,
	BATT_FULL_CHECK,
	DISABLE_CHARGER,
};
...
#define HIGH_BLOCK_TEMP		175
#define HIGH_RECOVER_TEMP		166
#define LOW_BLOCK_TEMP			 28
#define LOW_RECOVER_TEMP		 34
...
static bool batt_full_interrupt;
...
static struct device_attribute s3c_battery_attrs[] = {
	...
	SEC_BATTERY_ATTR(batt_volt),
	SEC_BATTERY_ATTR(batt_full_interrupt),
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = chg->bat_info.batt_vcell;
		break;
	case POWER_SUPPLY_PROP_CAPACITY:
		val->intval = chg->bat_info.batt_soc;
		break;
...
{
	int temp = 0;
	int temp_adc = chg->bat_info.batt_temp_adc;
	int health = chg->bat_info.batt_health;

	/* we think that our ADC_value is in Fahrenheit
	   convert it to Celsius and * 10 */
	temp = (temp_adc - 32) * 5 / 9 * 10;
	...
	} else if (temp_adc <= HIGH_RECOVER_TEMP && temp >= LOW_RECOVER_TEMP) {
		if (health == POWER_SUPPLY_HEALTH_OVERHEAT ||
...
static void s3c_bat_discharge_reason(struct chg_data *chg)
{
	...
	pr_debug("%s : Current Voltage : %d Current Level : %d%% \n	\
		Current time : %ld  discharging_time : %ld\n	\
		discharging reason : %d\n",			\
		__func__, chg->bat_info.batt_vcell / 1000, chg->bat_info.batt_soc, cur_time.tv_sec,
				   ...
				   const char *buf, size_t count)
{
	...
	case BATT_TEMP_ADC:
		if (sscanf(buf, "%d\n", &x) == 1) {
			chg->bat_info.batt_temp_adc = x;
			ret = count;
		}
		break;
	case BATT_SOC:
		if (sscanf(buf, "%d\n", &x) == 1) {
			chg->bat_info.batt_soc = x;
			ret = count;
		}
		break;
	case BATT_VOLT:
		if (sscanf(buf, "%d\n", &x) == 1) {
			chg->bat_info.batt_vcell = x;
			ret = count;
		}
		break;
	case BATT_FULL_INTERRUPT:
		if (sscanf(buf, "%d\n", &x) == 1) {
			batt_full_interrupt = x;
			ret = count;
		}
		break;
	case DISABLE_CHARGER:
...
static irqreturn_t max8998_int_work_func(int irq, void *max8998_chg)
{
	...
	if ((data & 0x4) || (ret != 0)) {
		...
		batt_full_interrupt = 1; // we will overwrite it from RIL after sending packet to modem
		chg->set_batt_full = 1;
...
static __devinit int max8998_charger_probe(struct platform_device *pdev)
{
	...
	chg->bat_info.batt_is_full = false;
	chg->bat_info.batt_temp_adc = 90;
	chg->bat_info.batt_temp = 100;
	chg->bat_info.batt_soc = 50;
	chg->bat_info.batt_vcell = 4000000;
	chg->set_charge_timeout = false;
